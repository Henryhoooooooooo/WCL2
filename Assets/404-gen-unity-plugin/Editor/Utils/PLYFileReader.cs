...
namespace GaussianSplatting.Editor.Utils
{
    public static class PLYFileReader
    {
        ...
        public static void ReadFile(string filePath, out int vertexCount, out int vertexStride, out List<string> attrNames, out NativeArray<byte> vertices, bool overrideVertexStride)
        {
            ...
            var tempVertices = new NativeArray<byte>(vertexCount * vertexStride, Allocator.Persistent);
            ...
            // Copy data and insert padding 0s 
            for (int i = 0; i < vertexCount; i++){
                int oldStartIndex = i * vertexStride;
                int newStartIndex = i * newVertexStride;
                
                // old 0, new 0
                for (int j=0; j < 12; j++){
                    vertices[newStartIndex+j] = tempVertices[oldStartIndex+j];
                }

                for (int j=0; j < 12; j++){
                    vertices[newStartIndex+j+12] = 0;
                }

                // old 12, new 24
                for (int j=0; j < 12; j++){
                    vertices[newStartIndex+j+24] = tempVertices[oldStartIndex+j+12];
                }

                for (int j=0; j < 180; j++){
                    vertices[newStartIndex+j+36] = 0;
                }

                // old 24, new 204
                for (int j=0; j < 32; j++){
                    vertices[newStartIndex+j+216] = tempVertices[oldStartIndex+j+24];
                }               
